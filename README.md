...
첫 자취 준비물 구매 결정을 시작점으로, 계약·이사·입주·생활·퇴실의 다음 행동과 검증된 연결처를 이어 주는 주거 생활 실행 시스템입니다. 현재 배포 범위는 보유품과 생활 조건을 반영한 구매 계획입니다.

제품의 최종 목표, 현재 범위, AI·제휴·법률 확장 원칙은 [제품 비전 문서](docs/product-vision.md)에 기록합니다.

현재 성장 루프, 측정식, 다음 서비스 연결 단계의 진입 조건은 [Stage 1 실행 문서](docs/stage-1-growth-loop-2026-09-02.md)에 기록합니다.

## 홍보 초안 생성

```bash
npm run promotion:build
```

네이버 블로그, 디스콰이엇, GeekNews, 인스타그램용 문구와 채널별 UTM 링크를 `data/exports/`에 생성합니다. 게시 전 사람이 확인하는 승인용 초안이며 외부 채널에 자동 게시하지 않습니다.
